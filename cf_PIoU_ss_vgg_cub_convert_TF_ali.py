...
def main():
    ...
    chosen_class_ids_full = np.arange(0, 200)
    chosen_classes = [label_map[i] for i in chosen_class_ids_full]
    imgDataGen_official_split = ImageDataGenerator(preprocessing_function = preprocess_input)
    test_gen_full  = imgDataGen_official_split.flow_from_directory(data_dir_test,
                    target_size=(input_shape[1], input_shape[2]),
                    color_mode='rgb',
                    class_mode='categorical',
                    batch_size=batch_size,
    ...
    cross_match = np.zeros((len(predicted_class), 2))
    ...
    val_loader = test_gen
    picked_list = np.asarray(val_loader.filepaths)
    
    im_list = np.asarray(test_gen_full.filepaths)
    
    remaining_mask_size_pool = np.arange(0.01, 0.5, 0.01)
    remaining_mask_size_pool = np.concatenate(([0.005], remaining_mask_size_pool))
    match_points_IOU = cf_proposal_extraction(test_gen_full, val_loader, heat_map_hp, heat_map_cls,
                                                                     picked_list, im_list,dis_extracted_attributes,
                                                                     picked_locations,all_locations, predicted_class,
                                                                     remaining_mask_size_pool,chosen_class_ids, cross_match,counterfactual_class)
...
def create_image_bank(val_loader, c_num, imlist):
    image_bank = np.zeros((c_num, 224, 224,3))
    indicator_vector = np.zeros((c_num))
...
def cf_proposal_extraction(val_loader_full, val_loader, heat_map_hp, heat_map_cls, imglist, im_list_full, dis_extracted_attributes, part_Locs,all_locations, predicted_class, remaining_mask_size_pool, chosen_class_ids, cross_match,counterfactual_class):

    image_bank, X_Y_bank, index_bank = create_image_bank(val_loader_full, 200, im_list_full)
        ...
        for i_batch in range(index.shape[0]):
            ...
            for i_remain in range(np.size(remaining_mask_size_pool)):
                ...
                if show_images:
                    ...
                    plt.imshow(np.squeeze(input[i_batch])), plt.axis('on'), plt.title('original image'), plt.show()
                    plt.plot(part_Locs_example_copy[:,0],part_Locs_example_copy[:,1],'g+',color='red')
                    plt.imshow(np.squeeze(cf_heatmap)), plt.axis('on'), plt.title('cf_heatmap'), plt.show()
        
                    plt.plot(part_Locs_example[dis_attributes,0],part_Locs_example[dis_attributes,1],'g+',color='red')
                    plt.imshow(np.squeeze(input[i_batch])), plt.axis('on'), plt.title('original image'), plt.show()
                    show_images = False
    ...
    gt_all_images = np.asarray(gt_all_images).flatten()
    for i in range(len(imglist)):

        if counterfactual_class[i]!=gt_all_images[i]:
                print("skipping misclassification")
                continue
        if i % 100 == 0:
            print('processing', i)
        ...
        distractor_img = np.expand_dims(distractor_img,0)
        ...
        for i_remain in range(np.size(remaining_mask_size_pool)):
            ...
            if show_images:
                plt.plot(part_Locs_example[:,0],part_Locs_example[:,1],'g+',color='red')
                plt.imshow(np.squeeze(distractor_img)), plt.axis('on'), plt.title('original image'), plt.show()
                plt.plot(part_Locs_example[:,0],part_Locs_example[:,1],'g+',color='red')
                plt.imshow(np.squeeze(cf_heatmap)), plt.axis('on'), plt.title('cf_heatmap'), plt.show()
                show_images = False
    ...
    print("missclassifications:", miclassifications)
