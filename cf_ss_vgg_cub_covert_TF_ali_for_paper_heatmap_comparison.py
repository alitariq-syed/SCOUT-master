...
def main():
    ...
    test_gen  = imgDataGen_official_split.flow_from_directory(data_dir_user_evaluation,
    ...
    np.save('./for_user_evaluation/all_predicted_te_cls_vgg16.npy', all_predicted_te)
    np.save('./for_user_evaluation/all_gt_target_te_cls_vgg16.npy', all_gt_target_te)
    np.save('./for_user_evaluation/all_top2_classes_cls_vgg16.npy', all_top2_classes)
    ...
    all_predicted_teacher = np.load('./for_user_evaluation/all_predicted_te_cls_vgg16.npy')
    all_gt_target_teacher = np.load('./for_user_evaluation/all_gt_target_te_cls_vgg16.npy')
    all_top2_classes_teacher = np.load('./for_user_evaluation/all_top2_classes_cls_vgg16.npy')
...
def validate_ali(val_loader, model_main, chosen_class_ids):
    ...
    #manually chosen
    all_top2_classes = np.array([25,108, 9,9,9,9,170,170,125,125, 9]) 
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        for i_batch in range(index.shape[0]):
            ...
            for i_remain in range(np.size(remaining_mask_size_pool)):
                ...
                remaining_mask_size = remaining_mask_size_pool[1]
                #maps = 'a'
                if maps == 'a':
                ...
                if show_images:
                    ...
                    # cam = show_cam_on_image(input_post_process, cf_heatmap)
                    ...
                    # heatmap_display = heatmap_gradcam_display(cf_heatmap_non_threshold, input_post_process)
                    # heatmap_display_thresholded = heatmap_gradcam_display(cf_heatmap, input_post_process)
    
                    # plt.imshow(cf_heatmap_non_threshold),plt.show()
                    # plt.imshow(cf_heatmap),plt.show()
                    # plt.imshow(heatmap_display),plt.axis('off'), plt.title(''), plt.savefig(fname="./for_user_evaluation/"+str(i_batch)+".png", dpi=300, bbox_inches = 'tight'), plt.show()
                    plt.imshow(heatmap_display),plt.axis('off'), plt.title(''), plt.savefig(fname="./for_user_evaluation/"+str(i_batch)+"_thresh_customtop2.png", dpi=300, bbox_inches = 'tight'), plt.show()
                    break
